 ...
 // 获取最新数据列表
    public ResultSet GetLastData_data(String orderby, int beginindex, int endindex, boolean unit,String nodelist){
        ...
        if (null != list && list.size() > 0){
            ...
            sb_sql.append("SELECT NodeNo AS 节点编号 , Location AS 安装地点 ,Longitude AS 经度,[Latitude] AS 纬度, [DATETIME] AS 更新时间 ,ishavevideo as 是否有摄像头,[OrderNumber] as OrderNumber,");
...
//            Log.v(tag, "^^^^^^^^^^^^^^^^2^" + sb_columns1.toString());
            sb_columns1.append(" FROM ( SELECT [Node].NodeNo , [Node].Location ,[Node].Longitude,[Node].[Latitude],[ResultData2].DATETIME ,OrderNumber, CAST([ResultData2].[SensorNo] AS VARCHAR) + '_' + CAST([ResultData2].[Channel] AS VARCHAR) AS [SensorList] , [ResultData2].DATA,ResultData2.ishavevideo  FROM (");
