            ...
            switch (msg.what) {
                case MSG_LOGIN_RETURN:  
                    ...
                    if (!TextUtils.isEmpty(uniqueID)) {
                        ...
                        Intent intent = new Intent(LoginActivity.this, JuRongActivity.class);
...
//                WindowManager.LayoutParams.FLAG_FULLSCREEN);
        ...
        if (mPref != null) {
            ...
            if (!TextUtils.isEmpty(pro_username)) {
                ...
                if (!TextUtils.isEmpty(userName) && ! TextUtils.isEmpty(password) && isJuRongUser(userName)){
//                    mWaitDialog = Utils.createLoadingDialog(LoginActivity.this, "正在登陆");
//                    mWaitDialog.show();
                    if (isFakeUser(userName)){
                        if (password.equalsIgnoreCase("123")){
                            uniqueID = "0909c545-422b-4c94-a085-c010e17b";
    ...
    protected void onResume() {
        ...
        logButton.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (isFakeUser(userName) && "123".equalsIgnoreCase(password)){
                    uniqueID = "0909c545-422b-4c94-a085-c010e17b";
                    mHandler.sendEmptyMessage(MSG_LOGIN_RETURN);
                    return;
                }
    ...
    private boolean isFakeUser(String name){
        if ("admin".equalsIgnoreCase(name) || "jr".equalsIgnoreCase(name)
                || "test".equalsIgnoreCase(name)){
            JuRongActivity.isFake = true;
            return true;
        }
        JuRongActivity.isFake = false;
        return false;
    }
    
    private boolean isJuRongUser(String name){
        if ("admin".equalsIgnoreCase(name) || "jr".equalsIgnoreCase(name)
                || "test".equalsIgnoreCase(name) || "jurong".equalsIgnoreCase(name)
                || "ruiqing".equalsIgnoreCase(name) || "angel".equalsIgnoreCase(name) 
                || "jrnw".equalsIgnoreCase(name)){
            return true;
        }
        Toast.makeText(this, "请使用句容用户的帐号！", Toast.LENGTH_SHORT).show();
        return false;
    }
